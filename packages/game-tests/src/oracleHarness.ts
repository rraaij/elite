...
export interface OracleFrameCapture {
	frame: number;
	tick: number;
	rngState: number;
...
export interface OracleScenarioCapture {
	scenarioId: string;
	seed: number;
	totalFrames: number;
	stepMs: number;
	frames: OracleFrameCapture[];
}
...
function captureFrame(frame: number, snapshot: SimulationSnapshot): OracleFrameCapture {
	return {
		frame,
		tick: snapshot.tick,
...
export function runOracleScenario(scenario: OracleScenario): OracleScenarioCapture {
	const simulation = createEmptySimulation({
		scenarioId: "empty",
		seed: scenario.seed,
	});
	const controls = createDefaultPilotControls();
	const frameToUpdate = new Map<number, Partial<PilotControlState>>();
	for (const keyframe of scenario.keyframes) {
		frameToUpdate.set(keyframe.frame, keyframe.updates);
	}

	const frames: OracleFrameCapture[] = [];
