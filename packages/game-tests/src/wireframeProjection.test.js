...
import { projectWireframeScene } from "../../game-renderer/src/index";

/**
 * Build one minimal blueprint with one edge.
 */
function createSingleEdgeBlueprint(shipId, start, end) {
	return {
		shipId,
		vertices: [start, end],
 ...
 */
function createBlueprintResolver(blueprints) {
	const map = new Map();
	for (const blueprint of blueprints) {
		map.set(blueprint.shipId, blueprint);
	}
	return (blueprintId) => map.get(blueprintId) ?? null;
}
