...
describe("canonical game state model", () => {
	...
	it("auto-transitions hyperspace after sustained high warp and updates system seeds", () => {
		...
		const previousCurrentSeed = [...state.universe.currentSystemSeed] as [number, number, number];
		const previousTargetSeed = [...state.universe.targetSystemSeed] as [number, number, number];
	...
	it("does not hyperspace jump when sustained warp has insufficient fuel", () => {
		...
		const seedBefore = [...state.universe.currentSystemSeed] as [number, number, number];
