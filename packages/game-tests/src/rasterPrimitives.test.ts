...
function createPointCollector(): {
  plot: PixelPlotter;
  toSortedPoints(): string[];
  hasOutOfBoundsPoint(bounds: RasterBounds): boolean;
} {
  const points: Array<{ x: number; y: number }> = [];
  const plot: PixelPlotter = (x: number, y: number): void => {
    points.push({ x, y });
  };
...
describe("renderer raster primitives", () => {
  it("clips pixel plotting to raster bounds", () => {
    const bounds: RasterBounds = { width: 4, height: 4 };
  ...
  it("draws horizontal lines with inclusive endpoints", () => {
    const bounds: RasterBounds = { width: 8, height: 6 };
    const collector = createPointCollector();
  ...
  it("draws the same pixel set when line endpoints are reversed", () => {
    const bounds: RasterBounds = { width: 8, height: 8 };
  ...
  it("draws radius-zero circles as one pixel", () => {
    const bounds: RasterBounds = { width: 10, height: 10 };
    const collector = createPointCollector();
  ...
  it("draws midpoint circle octants for radius two", () => {
    const bounds: RasterBounds = { width: 12, height: 12 };
    const collector = createPointCollector();
