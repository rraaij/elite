 ...
 */
export function plotPixel(bounds, x, y, plot) {
 ...
 */
export function drawLine(bounds, start, end, plot) {
	if (!hasDrawableArea(bounds)) {
		return 0;
	}
 ...
 * visible pixels.
 */
function plotUniqueCircleStepPoints(bounds, centerX, centerY, x, y, plot) {
 ...
 * Returns number of visible pixels emitted.
 */
export function drawCircle(bounds, center, radius, plot) {
	if (!hasDrawableArea(bounds)) {
		return 0;
	}
