import { buildCelestialScene } from "./celestialScene";
import {
...
import { createCockpitHudModel } from "./cockpitHud";
import { drawCircle, drawLine, plotPixel } from "./rasterPrimitives";
import {
...
import { projectWireframeScene } from "./wireframeProjection";
 ...
 */
function toRasterCoordinate(value) {
 ...
 */
function toRasterBounds(width, height) {
	return {
		width,
		height,
	};
 ...
 */
function createCanvasPlotter(ctx) {
	return (x, y) => {
 ...
 */
function drawFilledDisc(rasterBounds, plotter, centerX, centerY, radius) {
	if (radius <= 0) {
 ...
 */
function drawStarSample(ctx, rasterBounds, plotter, star) {
 ...
 */
function drawStarfield(ctx, rasterBounds, plotter, stars) {
 ...
 */
function drawCelestialDisc(ctx, rasterBounds, plotter, disc) {
	if (disc.kind === "sun") {
		// Outer glow.
 ...
 */
function drawCelestialBackdrop(ctx, rasterBounds, plotter, snapshot) {
 ...
 */
function toWireframeShipInstances(snapshot) {
 ...
 */
function drawProjectedSegments(rasterBounds, plotPixel, snapshot, wireframeConfig) {
	const projection = projectWireframeScene({
		...
		resolveBlueprintById: (blueprintId) => wireframeConfig.resolveBlueprintById(blueprintId),
	});
 ...
 */
function drawHudBar(ctx, bar, colorBehavior, x, y, width, height) {
 ...
 */
function drawHudBars(ctx, hudModel, colorBehavior) {
 ...
 */
function drawScannerContact(ctx, rasterBounds, plotter, contact, colorBehavior) {
 ...
 */
function drawScanner(ctx, rasterBounds, plotter, hudModel, colorBehavior) {
 ...
 */
function drawCompass(ctx, rasterBounds, plotter, hudModel, colorBehavior) {
 ...
 */
function drawIndicators(ctx, indicators, panelWidth, baseY, snapshot, colorBehavior) {
	const lampWidth = 32;
	const lampHeight = 10;
	const gap = 8;
 ...
 */
function drawOverlayLines(ctx, lines, colorBehavior, anchorX, startY, lineHeight) {
 ...
 */
function formatTransitionViewKey(viewKey) {
 ...
 */
function overlayTitleRgb(alertLevel) {
	switch (alertLevel) {
		case "caution":
			return [255, 236, 201];
		case "danger":
 ...
 */
function drawMixedViewTransitionOverlay(ctx, rasterBounds, transitionFrame, colorBehavior) {
 ...
 */
function drawCockpitTextLayers(ctx, rasterBounds, textLayerModel, transitionFrame, colorBehavior) {
	...
	if (!transitionFrame.active || transitionFrame.fromViewKey === null) {
		...
		ctx.fillStyle = `rgba(${titleRgb[0]}, ${titleRgb[1]}, ${titleRgb[2]}, ${previousAlpha.toFixed(3)})`;
		ctx.fillText(previousTitle, Math.floor(rasterBounds.width * 0.5) - slideOffset, 18);
		ctx.fillStyle = `rgba(${titleRgb[0]}, ${titleRgb[1]}, ${titleRgb[2]}, ${currentAlpha.toFixed(3)})`;
 ...
 */
function drawCockpitHud(ctx, rasterBounds, plotter, snapshot, colorBehavior) {
...
export function createCanvasRenderer(config) {
	...
	let logicalWidth = 0;
	let logicalHeight = 0;
	let viewTransitionTracker = createViewTransitionTracker();
	return {
