...
import type { HudBar, IndicatorLamp, ScannerContact } from "./cockpitHud";
import type { OverlayTextTone } from "./viewTextOverlay";

/**
 * High-level alert buckets used to shift HUD color treatment.
 *
 * The original game used compact palette signaling in the cockpit area.
 * We mirror that intent here by deriving a coarse alert level from flight
 * state and then mapping all render colors through one consistent palette.
 */
export type CockpitAlertLevel = "nominal" | "caution" | "danger";

/**
 * Frame-local flash cadence derived from the canonical HUD timer.
 *
 * We keep both slow and fast toggles so different systems can blink at
 * different rates (for example: ECM faster than warp/missile indicators).
 */
export interface HudFlashState {
	timerMs: number;
	slowOn: boolean;
	fastOn: boolean;
	pulse01: number;
...
/**
 * Resolved color palette consumed by canvas draw routines.
 *
 * Most fields map directly to a single visual region in the renderer so
 * updates stay data-driven instead of scattering color literals everywhere.
 */
export interface CockpitPalette {
	...
	scannerFrame: string;
	scannerGuide: string;
	scannerShipRgb: readonly [number, number, number];
	scannerDebrisRgb: readonly [number, number, number];
	compassFill: string;
	compassRing: string;
	compassNeedle: string;
	hudBarBackground: string;
	hudBarBorder: string;
	hudBarLabel: string;
	hudBarWarningOn: string;
	hudBarWarningOff: string;
	indicatorOff: string;
	indicatorBlinkOff: string;
	indicatorLabel: string;
	overlayHeader: string;
	overlayPrimary: string;
	overlayAccent: string;
	overlayDim: string;
	overlayWarningOn: string;
	overlayWarningOff: string;
	transitionSweep: string;
	transitionBand: string;
	reticle: string;
	headingPointer: string;
	diagnostics: string;
}

/**
 * Full color + flash behavior bundle consumed per render frame.
 */
export interface CockpitColorBehavior {
	alertLevel: CockpitAlertLevel;
	flash: HudFlashState;
 ...
 */
function normalizeTimerMs(timerMs: number): number {
 ...
 */
export function createHudFlashState(timerMs: number): HudFlashState {
 ...
 */
export function resolveCockpitAlertLevel(snapshot: SimulationSnapshot): CockpitAlertLevel {
 ...
 */
const NOMINAL_PALETTE: CockpitPalette = {
 ...
 */
const CAUTION_PALETTE: CockpitPalette = {
 ...
 */
const DANGER_PALETTE: CockpitPalette = {
 ...
 */
function paletteForAlertLevel(level: CockpitAlertLevel): CockpitPalette {
	switch (level) {
		case "caution":
 ...
 */
export function createCockpitColorBehavior(snapshot: SimulationSnapshot): CockpitColorBehavior {
 ...
 */
export function resolveHudBarFillColor(bar: HudBar, behavior: CockpitColorBehavior): string {
	if (!bar.warning) {
		return bar.color;
	}
 ...
 */
export function resolveIndicatorBlinkOn(
	indicatorKey: IndicatorLamp["key"],
	behavior: CockpitColorBehavior,
 ...
 */
export function resolveIndicatorLampColor(
 ...
 */
export function resolveOverlayToneColor(
 ...
 */
export function resolveScannerContactColor(
