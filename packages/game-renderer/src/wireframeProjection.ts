...
/**
 * Minimal 3D vector used by projection helpers.
 */
interface Vector3 {
	x: number;
	y: number;
...
/**
 * Minimal 2D vector used by viewport clipping helpers.
 */
interface Vector2 {
	x: number;
	y: number;
...
/**
 * Blueprint vertex shape consumed by the renderer.
 */
...
/**
 * Blueprint edge shape consumed by the renderer.
 */
...
/**
 * Runtime wireframe blueprint payload.
 *
 * Only fields needed for projection are modeled here; other metadata remains
 * in `game-data` and can be threaded in later phases as needed.
 */
...
/**
 * Ship instance data needed for wireframe projection.
 */
export interface WireframeShipInstance {
	slotId: number;
	kind: "ship" | "debris";
	blueprintId: number;
	position: Vector3;
...
/**
 * Viewport parameters for projection and clipping.
 */
export interface WireframeViewport {
	width: number;
	height: number;
...
/**
 * Projector configuration for one frame.
 */
export interface WireframeProjectionConfig {
	ships: readonly WireframeShipInstance[];
	headingDeg: number;
...
/**
 * One visible projected line segment emitted by the pipeline.
 */
export interface ProjectedWireframeSegment {
	slotId: number;
	blueprintId: number;
	edgeIndex: number;
	x0: number;
...
/**
 * Full projection output plus lightweight diagnostics.
 */
export interface WireframeProjectionResult {
	segments: ProjectedWireframeSegment[];
	diagnostics: {
		shipsConsidered: number;
		shipsProjected: number;
		edgesConsidered: number;
		edgesCulledByNearPlane: number;
		edgesCulledByViewport: number;
 ...
 */
function computeOutcode(point: Vector2, width: number, height: number): number {
...
function clipLineToViewport(
	...
	let pointA: Vector2 = { ...start };
	let pointB: Vector2 = { ...end };
 ...
 */
function rotateWorldToCameraByHeading(worldPoint: Vector3, headingDeg: number): Vector3 {
 ...
 */
function clipSegmentAgainstNearPlane(
	start: Vector3,
	end: Vector3,
	nearPlaneZ: number,
 ...
 */
function projectCameraPointToScreen(point: Vector3, viewport: WireframeViewport): Vector2 {
 ...
 */
function composeShipVertexWorldPosition(
 ...
 */
function normalizeViewport(viewport: WireframeViewport): WireframeViewport {
	const normalized: WireframeViewport = {
 ...
 */
export function projectWireframeScene(
	config: WireframeProjectionConfig,
	...
	const segments: ProjectedWireframeSegment[] = [];
